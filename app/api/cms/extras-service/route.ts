...
export async function GET() {
  try {
    ...
    const strapiData = await CMSAdapter.getServiceBySlug('extras-service');
    if (strapiData) {
    ...
    let data = await ExtrasService.findOne();

    if (!data) {
      data = await ExtrasService.create({});
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const updatedData = await ExtrasService.findOneAndUpdate(
      {},
