...
export async function GET() {
  try {
    ...
    const strapiData = await CMSAdapter.getServiceBySlug('moving-cleaning');
    if (strapiData) {
    ...
    let data = await MovingCleaning.findOne();

    if (!data) {
      data = await MovingCleaning.create({});
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const updatedData = await MovingCleaning.findOneAndUpdate(
      {},
