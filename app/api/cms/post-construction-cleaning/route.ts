...
export async function GET() {
  try {
    ...
    const strapiData = await CMSAdapter.getServiceBySlug('post-construction-cleaning');
    if (strapiData) {
    ...
    if (!data) {
      data = await PostConstructionCleaning.create({});
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const updatedData = await PostConstructionCleaning.findOneAndUpdate(
      {},
