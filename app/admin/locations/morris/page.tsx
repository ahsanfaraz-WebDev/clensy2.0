...
import MorrisCountyPage from "@/app/locations/morris/page"
import Image from "next/image"
import Link from "next/link"
import { signOut } from "next-auth/react"

interface MorrisForm {
  heroSection: {
    title: string
    subtitle: string
...
export default function MorrisLocationEditor() {
  const { data: session, status } = useSession()
  const router = useRouter()
  const [form, setForm] = useState<MorrisForm>(emptyForm)
  ...
  const updateField = (section: keyof MorrisForm, field: string, value: any) => {
    setForm(prev => ({
      ...prev,
      [section]: {
        ...prev[section],
  ...
  const addContent = () => {
    setForm(prev => ({
      ...
      aboutSection: {
        ...prev.aboutSection,
        content: [...prev.aboutSection.content, ""]
      }
    }))
  }
  ...
  const updateContent = (index: number, value: string) => {
    setForm(prev => ({
      ...
      aboutSection: {
        ...prev.aboutSection,
        content: prev.aboutSection.content.map((item, i) => 
          i === index ? value : item
        )
      }
    }))
  ...
  const deleteContent = (index: number) => {
    setForm(prev => ({
      ...
      aboutSection: {
        ...prev.aboutSection,
        content: prev.aboutSection.content.filter((_, i) => i !== index)
      }
    }))
  }
      ...
      <div className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        <div className="px-4 py-6 sm:px-0">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-3xl font-bold text-gray-900">Edit Morris County Location</h1>
            <button
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    {form.aboutSection.content.map((content, index) => (
