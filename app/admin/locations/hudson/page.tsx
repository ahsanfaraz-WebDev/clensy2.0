...
import HudsonCountyPage from "@/app/locations/hudson/page"
import Image from "next/image"
import Link from "next/link"
import { signOut } from "next-auth/react"

interface HudsonForm {
  heroSection: {
    title: string
    subtitle: string
...
export default function HudsonLocationEditor() {
  const { data: session, status } = useSession()
  const router = useRouter()
  const [form, setForm] = useState<HudsonForm>(emptyForm)
  ...
  const updateField = (section: keyof HudsonForm, field: string, value: any) => {
    setForm(prev => ({
      ...prev,
      [section]: {
        ...prev[section],
      ...
      <div className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        <div className="px-4 py-6 sm:px-0">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-3xl font-bold text-gray-900">Edit Hudson County Location</h1>
            <button
