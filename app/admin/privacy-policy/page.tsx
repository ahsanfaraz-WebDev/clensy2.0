...
export default function PrivacyPolicyAdmin() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (result.success) {
        ...
        toast({
          title: "Error",
          description: "Failed to fetch privacy policy data",
          variant: "destructive",
        });
      }
    } catch (error) {
  ...
  const handleSave = async () => {
    try {
      ...
      if (result.success) {
        ...
        toast({
          title: "Error",
          description: result.error || "Failed to update privacy policy",
          variant: "destructive",
        });
      }
    } catch (error) {
  ...
  const handleSectionChange = (index: number, field: keyof PrivacyPolicySection, value: string) => {
    ...
    <div className="min-h-screen p-4 bg-gray-50">
      <div className="max-w-4xl mx-auto space-y-8">
        <div className="bg-white p-6 rounded-lg shadow">
          <h1 className="text-2xl font-bold mb-6">Edit Privacy Policy</h1>
          ...
          {/* SMS Consent */}
          <div className="mb-8">
            <h2 className="text-xl font-semibold mb-4">SMS Consent</h2>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Description
                ...
                <textarea
                  value={data.smsConsent.description}
                  onChange={(e) => setData({
                    ...data,
                    smsConsent: { ...data.smsConsent, description: e.target.value }
                  })}
                  className="w-full p-2 border rounded-md"
                  rows={4}
                />
              ...
              <div>
                ...
                <textarea
                  value={data.smsConsent.optOutInstructions}
                  onChange={(e) => setData({
                    ...data,
                    smsConsent: { ...data.smsConsent, optOutInstructions: e.target.value }
                  })}
                  className="w-full p-2 border rounded-md"
                  rows={2}
                />
